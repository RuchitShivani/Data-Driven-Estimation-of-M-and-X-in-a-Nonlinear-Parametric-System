...
Data-Driven Estimation of θ, M, and X in a Nonlinear Parametric System
...
Yeah, right measure how close the predicted curve is to the real data, an error is calculated for each data point. For every t, the predicted values (x_pred,y_pred)from the equations are found and compare them with the given values (x_data,y_data). The difference for each point is found using the L1 distance, also called the Manhattan distance (extremely important):
"Error"=∣x_pred-x_data∣+∣y_pred-y_data∣

It is done for all the data points and add up all the errors to get a total error value.
Then, it is divided by the total by the number of points to find the average error per point.
...
Ultimately, an optimization method is used to automatically adjust θ, M, and X until this average error becomes as small as possible within the given ranges of 6-60. When the optimizer finishes, the best-fitting values of θ, M, and X are found and these values can then be used to plot the final fitted curve and compare it visually with the data points.
...
Methodology
	...
	Importing Libraries: First the required Python libraries are imported :
	...
	Generating Parameter Values (t): The variable t is the parameter that runs the curve.
Since tranges from 6 to 60, we generate equally spaced values using NumPy’s linspace function. This gives one t value for each data point in the CSV file.
	...
	Defining the Curve Function: This is the function that calculates x(t)and y(t)for any given set of parameters θ, M, and X. This function directly implements the equations given in the problem statement. The angle θ is converted from degrees to radians before using trigonometric functions because Python uses radians by default.

	Defining the Objective (Error) Function: This is another function that defines measuring how far the predicted curve is from the real data. This function calculates the L1 distance (sum of absolute differences) between predicted and actual x and y values.
The smaller this number, the closer the fit.

	Optimization Process The scipy.optimize.minimize function is used to automatically find the values of θ, M, and Xthat make the L1 error as small as possible.
Bounds for each parameter are provided so that the optimizer only searches within valid ranges:
	0°<θ<50°
	-0.05<M<0.05
	0<X<100

	Evaluating the Fit: After the optimizer finds the best parameters, calculation is done for:
	...
	Visualization: Plotting the original data points and the fitted curve together using Matplotlib. This helps visually confirm how well the fitted curve matches the actual data. A close overlap between the two indicates a successful fit.
 <img width="843" height="682" alt="image" src="https://github.com/user-attachments/assets/fed5044c-1a19-43ae-930b-6ad2fa06fee8" />
...
<img width="940" height="446" alt="image" src="https://github.com/user-attachments/assets/e5001f0f-8e90-419b-b4c0-747a9961e9bb" />
